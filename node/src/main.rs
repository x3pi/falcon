...
async fn main() {
    let matches = App::new(crate_name!())
        ...
        .subcommand(
            SubCommand::with_name("run")
                ...
                .args_from_usage("--executor-socket=[PATH] 'The path to the Unix Domain Socket for the executor'"),
        )
        .subcommand(
    ...
    match matches.subcommand() {
        ...
        ("run", Some(subm)) => {
            ...
            let executor_socket = subm.value_of("executor-socket").map(|s| s.to_string());

            match Node::new(committee_file, key_file, store_path, parameters_file, executor_socket).await {
                Ok(mut node) => {
    ...
    keys.iter()
        ...
        .map(|(i, keypair)| {
            ...
            Ok(tokio::spawn(async move {
                match Node::new(committee_file, &key_file, &store_path, None,None).await {
                    Ok(mut node) => {
