...
impl Node {
    pub async fn new(
        ...
        let (tx_commit, rx_commit) = channel(20000); //commit channel
        let (tx_consensus, rx_consensus) = channel(40000); // 协议交流消息
        let (tx_consensus_mempool, rx_consensus_mempool) = channel(4000);
